...
async function callAI_OpenAI(messages, cfg, includeTools = true) {
    ...
    if (cfg.OPENAI_BASE_URL?.includes('openrouter')) {
        headers['HTTP-Referer'] = 'http://localhost:30010';
...
async function callAI_Anthropic(messages, cfg, includeTools = true) {
    ...
    const apiKey = cfg.ANTHROPIC_API_KEY || cfg.ANTHROPIC_AUTH_TOKEN;
    const baseUrl = cfg.ANTHROPIC_BASE_URL || 'https://api.anthropic.com/v1';
    // Check if using MiniMax API
    ...
    // MiniMax API uses Authorization header
    if (isMiniMax) {
        headers['Authorization'] = `Bearer ${apiKey}`;
    } else {
        headers['x-api-key'] = apiKey;
    }
    const resp = await fetchExternal(`${baseUrl}/messages`, headers, body, 'POST');
...
function resolveProvider(cfg) {
    ...
    // custom provider: check CLAUDE_CODE_USE_OPENAI to decide routing
    if (cfg.CLAUDE_CODE_USE_OPENAI === '0') return 'anthropic';
    return 'openai';
}
...
async function streamChatResponse(messages, cfg, res) {
    const rawProvider = cfg.AI_PROVIDER;
    const provider = resolveProvider(cfg);
    const model = provider === 'anthropic'
        ? (cfg.ANTHROPIC_MODEL || cfg.AI_DISPLAY_MODEL)
        : (cfg.OPENAI_MODEL || cfg.AI_DISPLAY_MODEL);
    const baseUrl = provider === 'anthropic'
        ? (cfg.ANTHROPIC_BASE_URL || 'https://api.anthropic.com/v1')
        : (cfg.OPENAI_BASE_URL || 'https://api.openai.com/v1');
    const apiKey = cfg.OPENAI_API_KEY || cfg.GEMINI_API_KEY || cfg.ANTHROPIC_API_KEY || cfg.ANTHROPIC_AUTH_TOKEN;
    ...
    if (provider === 'anthropic') {
        ...
        const isMiniMax = baseUrl.includes('minimaxi.com');
        const headers = { 'Content-Type': 'application/json', 'anthropic-version': '2023-06-01', 'Content-Length': Buffer.byteLength(body) };
        if (isMiniMax) {
            headers['Authorization'] = `Bearer ${apiKey}`;
        } else {
            headers['x-api-key'] = apiKey;
        ...
        let fullText = '';
        await streamExternal(`${baseUrl}/messages`, headers, body,
            (chunk) => {
                chunk.split('\n').forEach(line => {
                    if (!line.startsWith('data: ')) return;
                    try {
...
const server = createServer(async (req, res) => {
    ...
    try {
        ...
        if (url.pathname === '/api/verify-key' && req.method === 'POST') {
            ...
            else if (provider === 'custom') { valid = true; }
