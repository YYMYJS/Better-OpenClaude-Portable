...
# Portable data — set early so npm install and engine both use USB paths
...
setup_provider() {
    ...
    echo -e "  ${CYAN}8)${RESET} ${BOLD}Custom${RESET}       ${DIM}- Custom OpenAI/Anthropic compatible API${RESET}"
    echo ""

    while true; do
        read -p "  Select your provider (1-8): " PROVIDER_SEL
        case "$PROVIDER_SEL" in
            ...
            8) setup_custom; return ;;
            *) echo -e "  ${RED}[ERROR] Invalid selection. Please choose 1-8.${RESET}" ;;
        esac
    done
}
...
setup_custom() {
    echo ""
    echo -e "  ${CYAN}--- CUSTOM PROVIDER SETUP ---${RESET}"
    echo ""
    echo -e "  ${CYAN}1)${RESET} OpenAI-compatible API"
    echo -e "  ${CYAN}2)${RESET} Anthropic-compatible API"
    echo ""
    read -p "  Select API type (1 or 2): " API_TYPE
    if [ "$API_TYPE" = "1" ]; then
        custom_openai
...
custom_openai() {
    echo ""
    read -p "  Enter API Base URL (e.g., https://api.example.com/v1): " USER_API_URL
    [ -z "$USER_API_URL" ] && echo -e "  ${RED}[ERROR] URL cannot be empty!${RESET}" && custom_openai && return
    echo ""
    read -p "  Enter your API Key: " USER_API_KEY
    [ -z "$USER_API_KEY" ] && echo -e "  ${RED}[ERROR] Key cannot be empty!${RESET}" && custom_openai && return
    echo -e "  ${DIM}Key: $(mask_key "$USER_API_KEY")${RESET}"
    echo ""
    read -p "  Enter Model Name (e.g., gpt-4o): " USER_MODEL
    [ -z "$USER_MODEL" ] && echo -e "  ${RED}[ERROR] Model name cannot be empty!${RESET}" && custom_openai && return
    save_env "AI_PROVIDER=custom
CLAUDE_CODE_USE_OPENAI=1
OPENAI_API_KEY=${USER_API_KEY}
OPENAI_BASE_URL=${USER_API_URL}
OPENAI_MODEL=${USER_MODEL}
AI_DISPLAY_MODEL=${USER_MODEL}"
}
...
custom_anthropic() {
    echo ""
    read -p "  Enter API Base URL (e.g., https://api.anthropic.com/v1): " USER_API_URL
    [ -z "$USER_API_URL" ] && echo -e "  ${RED}[ERROR] URL cannot be empty!${RESET}" && custom_anthropic && return
    echo ""
    read -p "  Enter your API Key: " USER_API_KEY
    [ -z "$USER_API_KEY" ] && echo -e "  ${RED}[ERROR] Key cannot be empty!${RESET}" && custom_anthropic && return
    echo -e "  ${DIM}Key: $(mask_key "$USER_API_KEY")${RESET}"
    echo ""
    read -p "  Enter Model Name (e.g., claude-3-5-sonnet): " USER_MODEL
    [ -z "$USER_MODEL" ] && echo -e "  ${RED}[ERROR] Model name cannot be empty!${RESET}" && custom_anthropic && return
    save_env "AI_PROVIDER=custom
CLAUDE_CODE_USE_OPENAI=0
ANTHROPIC_AUTH_TOKEN=${USER_API_KEY}
ANTHROPIC_BASE_URL=${USER_API_URL}
...
if [ -n "$AI_PROVIDER" ]; then
    ...
    # Map custom to the right provider flag
    if [ "$AI_PROVIDER" = "custom" ]; then
        if [ "$CLAUDE_CODE_USE_OPENAI" = "1" ] || [ "$CLAUDE_CODE_USE_OPENAI" = "" ]; then
            OC_PROVIDER="openai"
        else
...
if [ -f "$ENGINE_DIR/win32-rm-patch.cjs" ]; then
    export NODE_OPTIONS="--require $ENGINE_DIR/win32-rm-patch.cjs"
fi
