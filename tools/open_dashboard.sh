...
if lsof -i :3000 >/dev/null 2>&1 || lsof -i :3000 2>/dev/null | grep -q ':3000 ' || lsof -i :3000 >/dev/null 2>&1; then
    ...
    if [ "$PORT_CHOICE" = "1" ]; then
        open "http://localhost:30010" 2>/dev/null &
...
echo -e "  ${DIM}Dashboard will be available at ${BOLD}http://localhost:30010${RESET}"
echo ""

# Open browser
if command -v open &>/dev/null; then
    (sleep 1 && open "http://localhost:30010") &
elif command -v termux-open-url &>/dev/null; then
    (sleep 1 && termux-open-url "http://localhost:30010") &
elif command -v xdg-open &>/dev/null; then
    (sleep 1 && xdg-open "http://localhost:30010") &
fi
